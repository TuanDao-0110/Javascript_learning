...
const searchResult = (inputValueString) => {
    let resultValue = []
    data.map((item, index) => {
        if (item.spacecraft.toLowerCase().includes(inputValueString.toLowerCase())) {
            resultValue.push(item.spacecraft)
        }
    })
    return resultValue
}
...
let input = document.querySelector('#app-search')
input.addEventListener('keyup', () => {
    let arrValue = searchResult(input.value)
    console.log(arrValue)
    renderValue(arrValue)
})
